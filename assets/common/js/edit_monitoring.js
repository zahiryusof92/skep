$(document).ready(function () {
    ...
    // minutes_ajk_ocr
    $('body').delegate('#minutes_ajk_ocr', 'change', function () {
        $('#upload_minutes_ajk_ocr').ajaxForm({
            dataType: 'json',
            beforeSubmit: function () {
                $("#minutes_ajk_ocr_error").empty().hide();
                return true;
            },
            success: function (result) {
                if (result.success) {
                    $("#minutes_ajk_ocr_error").html("<i class='fa fa-check' style='color:green;'></i>&nbsp;&nbsp;<button type='button' class='btn btn-xs btn-danger' onclick='clearMinutesAjkOcr()'><i class='fa fa-times'></i> </button>").show();
                    $("#minutes_ajk_ocr_clear").show();
                    $("#minutes_ajk_ocr").css("color", "green");
                    $("#minutes_ajk_ocr_url").val(result.file);
                } else {
                    var arr = result.errors;
                    $.each(arr, function (index, value) {
                        if (value.length != 0) {
                            $("#minutes_ajk_ocr_error").append('<span style="color:red;font-style:italic;font-size:13px;">' + value + '<span>');
                        }
                    });
                    $("#minutes_ajk_ocr_error").show();
                    $("#minutes_ajk_ocr").css("color", "red");
                }
            },
        }).submit();
    });
...
function clearMinutesAjkOcr() {
    $("#minutes_ajk_ocr").val("");
    $("#minutes_ajk_ocr_url").val("");
    $("#minutes_ajk_ocr").css("color", "grey");
    $("#minutes_ajk_ocr_error").hide();
}
