...
<div class="row padding-vertical-10">
    <div class="col-lg-12">
        ...
        <fieldset disabled>
            ...
            <div class="row">
                <table class="table table-sm borderless" id="old_dynamic_form_staff" style="font-size: 12px;">
                    <thead>
                        <tr>
                            <th width="5%">&nbsp;</th>
                    ...
                    <tbody>
                        ...
                        @foreach ($staffOldFile as $item)
                        <?php
                        $gaji_per_person = $item->gaji_per_orang;
                        $bil_pekerja = $item->bil_pekerja;
                        $total_gaji = $item->gaji_per_orang * $item->bil_pekerja;
                        $total_all_gaji += $item->gaji_per_orang * $item->bil_pekerja;
                        ...
                        <tr id="staffOld_row{{ ++$count }}">
                            ...
                            <td><input type="currency" id="{{ $old_prefix . 'gaji_per_orang_' . $count }}" name="{{ $old_prefix }}gaji_per_orang[]" class="form-control form-control-sm text-right" value="{{ $item->gaji_per_orang }}"></td>
                            <td><input type="digit" id="{{ $old_prefix . 'bil_pekerja_' . $count }}" name="{{ $old_prefix }}bil_pekerja[]" class="form-control form-control-sm text-right" value="{{ $item->bil_pekerja }}"></td>
                            <td><input type="currency" id="{{ $old_prefix . 'total_gaji_' . $count }}" name="{{ $old_prefix }}total_gaji[]" class="form-control form-control-sm text-right" value="{{ number_format($total_gaji, 2, '.', '') }}"></td>
                        ...
                        <tr>
                            ...
                            <th><input type="currency" id="{{ $old_prefix . 'total_gaji' }}" class="form-control form-control-sm text-right" value="{{ number_format($total_all_gaji, 2, '.', '') }}"></th>
