@if (count($attributes) > 0)
    @foreach ($attributes as $attribute)
        @if (isset($management) && !empty($management))
            ...
            @elseif ($fields[$attribute]['name'] == 'management_email')
                <?php $preset_val = $management['email']; ?>
            @endif
        @endif
                ...
                <div class="form-group">
                    ...
                    @if ($fields[$attribute]['type'] == 'textarea')
                        <textarea type="text" id="{{ $fields[$attribute]['name'] }}" name="{{ $fields[$attribute]['name'] }}"
                            class="form-control" rows="5">{{ !empty(Input::old($fields[$attribute]['name'])) ? Input::old($fields[$attribute]['name']) : (isset($model[$fields[$attribute]['name']]) ? $model[$fields[$attribute]['name']] : '') }}</textarea>
                        ...
                        <div class="radio">
                            @if ($fields[$attribute]['options'])
                                @foreach ($fields[$attribute]['options'] as $value => $name)
                                    <label>
                                        <input type="radio" name="{{ $fields[$attribute]['name'] }}"
                                            value="{{ $value }}"
                                            {{ Input::old($fields[$attribute]['name']) == $value
                                                ? 'checked'
                                                : (isset($model[$fields[$attribute]['name']])
                                                    ? ($model[$fields[$attribute]['name']] == $value
                                                        ? 'checked'
                                                        : '')
                                                    : '') }} />
                                        {{ $name }}
                    ...
                    @elseif ($fields[$attribute]['type'] == 'date')
                        <label class="input-group">
                            <input type="text" id="{{ $fields[$attribute]['name'] }}"
                                name="{{ $fields[$attribute]['name'] }}" class="form-control date_picker"
                    ...
                    @elseif ($fields[$attribute]['type'] == 'file')
                        <br />
                        <input type="file" id="{{ $fields[$attribute]['name'] }}_tmp"
                            name="{{ $fields[$attribute]['name'] }}_tmp" onChange="onUpload(this)" />
