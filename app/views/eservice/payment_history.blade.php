            ...
            <section class="panel panel-pad">
                <div class="row padding-vertical-20">
                    ...
                    <div class="col-lg-12">
                        ...
                        <table class="table table-hover nowrap table-own table-striped" id="eservice_order_table"
                            width="100%">
                            <thead>
                                <tr>
                                    <th style="width:20%;">{{ trans('app.forms.created_at') }}</th>
                                    <th style="width:30%;">{{ trans('app.forms.order_no') }}</th>
                                    <th style="width:15%;">{{ trans('app.forms.payment_method') }}</th>
                                    <th style="width:20%;">{{ trans('app.forms.amount') }} (RM)</th>
...
<script>
    $(document).ready(function () {
        let oTable = $('#eservice_order_table').DataTable({
            processing: true,
            serverSide: true,
            ajax: {
                'url' : "{{ route('eservice.paymentHistory') }}",
                'data': function(data) {
            ...
            responsive: true,
            columns: [
                {data: 'payment_created_at', name: 'eservices_order_transactions.payment_created_at'},
                {data: 'order_no', name: 'eservices_orders.order_no'},
                {data: 'payment_method', name: 'eservices_order_transactions.payment_method'},
                {data: 'payment_amount', name: 'eservices_order_transactions.payment_amount'},
                {data: 'status', name: 'eservices_order_transactions.status'},
