                        ...
                        <form id="file-movement-form" class="form-horizontal" onsubmit="event.preventDefault();">
                            ...
                            <div class="row">
                                <div class="col-lg-6">
                                    <div class="form-group">
                                        <label class="form-control-label">{{ trans('app.forms.name') }}</label><br />
                                        <select id="strata" name="strata" class="form-control"
                                            data-placeholder="{{ $model->strata? $model->strata : trans('app.forms.please_select') }}">
                                ...
                                <div class="col-lg-6">
                                    <div class="form-group {{ $errors->has('file') ? 'has-danger' : '' }}">
                                        ...
                                        <select id="file_id" name="file_id" class="form-control"
                                            data-placeholder="{{ $model->file_id? $model->file->file_no : trans('app.forms.please_select') }}">
                                            <option value="">{{ trans('app.forms.please_select') }}</option>
                                        </select>
                                        @include('alert.feedback', ['field' => 'file'])
                                    </div>
                                </div>
                            </div>
                            ...
                            <div id="main-container">
                                ...
                                <div class="container-item">
                                    <div class="row">
                                        <div class="col-lg-6">
                                            <div class="form-group {{ $errors->has(" assigned_to_$key") ? 'has-danger'
                                                : '' }}">
                                                <label class="form-control-label"><span style="color: red;">*</span> {{
                                                    trans('app.forms.assigned_to') }} <span id="lbl_date_{{$key}}">({{
                                                        !empty($val['value'])? $val['created_at'] : ''
                                                        }})</span></label>
                                                <button class="remove-item btn btn-danger btn-xs">{{ trans('Remove')
                                                    }}</button>
                                                <select id="assigned_to_{{$key}}" name="assigned_to[]"
                                                    class="form-control select3">
                                                    <option value="">{{ trans('app.forms.please_select') }}</option>
                                                    @foreach($userList as $user)
                                                    <option value="{{$user->id}}" {{ ($val['value'])==$user->id ?
                                                        'selected' : '' }}>{{ ucfirst($user->full_name) }}</option>
                                                    @endforeach
                                                </select>

                                                @include('alert.feedback-ajax', ['field' => "assigned_to_$key"])
                                            </div>
                                            <input id="created_at_{{$key}}" name="created_at[]"
                                                value="{{$val['created_at']}}" hidden>
                                            @include('alert.feedback-ajax', ['field' => "created_at_$key"])
                                        </div>
                                    </div>
                                </div>
                                @endforeach
                                ...
                                <div class="container-item">
                                    <div class="row">
                                        <div class="col-lg-6">
                                            ...
                                            @include('alert.feedback-ajax', ['field' => 'created_at_0'])
                                        </div>
                                    </div>
                                </div>
                            </div>
...
<script>
    $(function() {
        ...
        $("#submit_button").click(function (e) {
            ...
            let route = "{{ route('file-movement.update', [':id']) }}";
            let url = route.replace(':id', "{{ \Helper\Helper::encode(Config::get('constant.module.file_movement.name'), $model->id) }}");
            ...
            $.ajax({
                ...
                beforeSend: function() {
                    ...
                    $.each($('form').serializeArray(), function (key, value) {
                        if(value['name'].includes('assigned_to')) {
                            ...
                            $("#created_at_" + c + "_error").children("strong").text("");
                ...
                success: function (res) {
                    if (res.success == true) {
                        bootbox.alert("<span style='color:green;'>{{ trans('app.successes.file_movement.update') }}</span>", function () {
