...
<input id="cob" name="cob" value="{{ Str::lower($cobLetter->company->short_name) }}" hidden>
<div class="form-group row">
    <div class="col-md-5">
    ...
    <div class="col-md-7">
        <select id="type" name="type" class="form-control select3">
            @foreach($types as $type)
                <option value="{{ $type['name'] }}" {{ $cobLetter->type == $type['name']? "selected" : "" }}>{{ $type['title'] }}</option>
            @endforeach
        </select>
    ...
    function getFields(type) {
        ...
        $.ajax({
            ...
            data: {
                cob: $('#cob').val(),
                type: type,
                id: "{{ \Helper\Helper::encode(Config::get('constant.module.cob_letter.name'), $cobLetter->id) }}"
            },
            beforeSend: function() {
                $("#loading").css("display", "inline-block");
                $("#modal_submit_button").attr("disabled", "disabled");
