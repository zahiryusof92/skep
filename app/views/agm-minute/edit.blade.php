            ...
            <section class="panel panel-pad">
                <div class="row padding-vertical-20">
                    <div class="col-lg-12">
                        ...
                        <form id="minute-form" class="form-horizontal" method="POST" action="{{ route('agm-minute.update', \Helper\Helper::encode(Config::get('constant.module.agm.minute.name'), $model->id)) }}">
                            ...
                            <div class="form-group row">
                                ...
                                <div class="col-md-6">
                                    <select id="type" name="type" class="form-control select2">
                                        ...
                                        <option value="jmb" {{ (Input::old('type') == 'jmb' || $model->type == 'jmb') ? 'selected' : '' }}>{{ strtoupper(trans('jmb')) }}</option>
                                        <option value="mc" {{ (Input::old('type') == 'mc' || $model->type == 'mc') ? 'selected' : '' }}>{{ strtoupper(trans('mc')) }}</option>
                                    </select>
                            ...
                            <div class="form-group row">
                                ...
                                <div class="col-md-6">
                                    <select id="agm_type" name="agm_type" class="form-control select2">
                                        <option value="">{{ trans('app.forms.please_select') }}</option>   
                                        <option value="agm" {{ (Input::old('agm_type') == 'agm' || $model->agm_type == 'agm') ? 'selected' : '' }}>{{ strtoupper(trans('agm')) }}</option>
                                        <option value="egm" {{ (Input::old('agm_type') == 'egm' || $model->agm_type == 'egm') ? 'selected' : '' }}>{{ strtoupper(trans('egm')) }}</option>
                                    </select>
                                ...
                                <div class="col-md-6">
                                    <div class="form-group {{ $errors->has('file_no') ? 'has-danger' : '' }}">
                                        <select id="file_no" name="file_no" class="form-control select2">
                                            ...
                                            <option value="{{ \Helper\Helper::encode(Config::get('constant.module.cob.file.name'), $file->id) }}" {{ (Input::old('file_no') == $file->id || $model->file_id == $file->id) ? 'selected' : '' }}>{{ $file->file_no }}</option>
                                            @endforeach
                                        </select>
                                ...
                                <div class="col-md-6">
                                    <div class="form-group {{ $errors->has('agm_date') ? 'has-danger' : '' }}">
                                        <label class="input-group">
                                            <input type="text" class="form-control datepicker-only-init" placeholder="{{ trans('app.forms.agm_date') }}" id="agm_date_raw" name="agm_date" value=""/>
                            ...
                            <div id="form_container">
                                @include('agm-minute.form', compact('model', 'questions'))
                            </div>
...
<script>
    $(function() {
        ...
        $("#minute-form").submit(function (e) {
            ...
            let route = "{{ route('agm-minute.update', [':id']) }}";
            let url = route.replace(':id', "{{ \Helper\Helper::encode(Config::get('constant.module.agm.minute.name'), $model->id) }}");
            ...
            $.ajax({
                ...
                success: function (res) {
                    if (res.success == true) {
                        bootbox.alert("<span style='color:green;'>{{ trans('app.successes.minutes.update') }}</span>", function () {
