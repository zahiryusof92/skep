...
class ImportFile
{
    public function fire($job, $data)
    {
        ...
        // $exist_ref_file = Files::where('company_id', $company_id)->where('ref_no', $ref_no)->where('is_deleted', 0)->first();
                                
        // if(empty($exist_ref_file)) {
            ...
            // 160. Status
            $is_active = 0;
            if (isset($row['160']) && !empty($row['160'])) {
                $is_active_raw = trim($row['160']);
            ...
            if ($create_or_update_file) {
                ...
                if (isset($row['20']) && !empty($row['20'])) {
                    $strata_name = trim($row['20']);
                }
                // 21. Parliament
                $parliament = '';
                if (isset($row['21']) && !empty($row['21'])) {
                    $parliament_raw = trim($row['21']);
                ...
                if (isset($row['24']) && !empty($row['24'])) {
                    $strata_address1 = trim($row['24']);
                }
                // 25. Address 2
                $strata_address2 = '';
                if (isset($row['25']) && !empty($row['25'])) {
                    $strata_address2 = trim($row['25']);
                }
                // 26. Address 3
                $strata_address3 = '';
                if (isset($row['26']) && !empty($row['26'])) {
                    $strata_address3 = trim($row['26']);
                }
                // 27. Address 4
                $strata_address4 = '';
                if (isset($row['27']) && !empty($row['27'])) {
                    $strata_address4 = trim($row['27']);
                }
                // 28. Postcode
                $strata_postcode = '';
                if (isset($row['28']) && !empty($row['28'])) {
                    $strata_postcode = trim($row['28']);
                }
                // 29. City
                $strata_city = '';
                if (isset($row['29']) && !empty($row['29'])) {
                    $strata_city_raw = trim($row['29']);
                ...
                if (isset($row['31']) && !empty($row['31'])) {
                    $strata_country_raw = trim($row['31']);
                ...
                if (isset($row['38']) && !empty($row['38'])) {
                    $land_area = trim($row['38']);
                }
                // 39. Total Land Area UOM
                $land_area_unit = '';
                if (!empty($land_area)) {
                    if (isset($row['39']) && !empty($row['39'])) {
                        $land_area_unit_raw = trim($row['39']);
                ...
                // 41. Vacant Possession Date
                $vacant_date = '';
                if (isset($row['41']) && !empty($row['41'])) {
                    if(!empty($row['41']['date'])) {
                        $vacant_date = Carbon::parse($row['41']['date'])->format('Y-m-d');
                    } else {
                        $vacant_date = Carbon::createFromFormat('d/m/Y', trim($row['41']))->format('Y-m-d');
                    }
                }
                // 42. Date CCC
                $ccc_date = '';
                if (isset($row['42']) && !empty($row['42'])) {
                    if(!empty($row['42']['date'])) {
                        $ccc_date = Carbon::parse($row['42']['date'])->format('Y-m-d');
                    } else {
                        $ccc_date = Carbon::createFromFormat('d/m/Y', trim($row['42']))->format('Y-m-d');
                    }
                }
                // 43. CCC No.
                $ccc_no = '';
                ...
                if (isset($row['47']) && !empty($row['47'])) {
                    $total_share_unit = trim($row['47']);
                }
                // 48. Residential
                $is_residential = 0;
                if (isset($row['48']) && !empty($row['48'])) {
                    $is_residential_raw = trim($row['48']);
                ...
                if ($create_strata) {
                    if ($strata->is_residential) {
                        ...
                        if (isset($row['49']) && !empty($row['49'])) {
                            $residential_unit_no = trim($row['49']);
                        }
                        // 50. Parking Lot
                        $residential_parking_lot = 0;
                        if (isset($row['50']) && !empty($row['50'])) {
                            $residential_parking_lot = trim($row['50']);
                        }
                        // 51. Maintenance Fee
                        $residential_mf = '';
                        if (isset($row['51']) && !empty($row['51'])) {
                            $residential_mf = trim($row['51']);
                        }
                        // 52. Maintenance Fee UOM
                        $residential_mf_unit = '';
                        if (!empty($residential_mf)) {
                            if (isset($row['52']) && !empty($row['52'])) {
                                $residential_mf_unit_raw = trim($row['52']);
                        ...
                        if (isset($row['53']) && !empty($row['53'])) {
                            $residential_sf = trim($row['53']);
                        }
                        // 54. Singking Fund UOM
                        $residential_sf_unit = '';
                        if (!empty($residential_sf)) {
                            if (isset($row['54']) && !empty($row['54'])) {
                                $residential_sf_unit_raw = trim($row['54']);
                        ...
                        $residential->parking_lot = $residential_parking_lot;
                    ...
                    if ($strata->is_commercial) {
                        ...
                        if (isset($row['56']) && !empty($row['56'])) {
                            $commercial_unit_no = trim($row['56']);
                        }
                        // 57. Parking Lot
                        $commercial_parking_lot = 0;
                        if (isset($row['57']) && !empty($row['57'])) {
                            $commercial_parking_lot = trim($row['57']);
                        }
                        // 58. Maintenance Fee
                        $commercial_mf = '';
                        if (isset($row['58']) && !empty($row['58'])) {
                            $commercial_mf = trim($row['58']);
                        }
                        // 59. Maintenance Fee UOM
                        $commercial_mf_unit = '';
                        if (!empty($commercial_mf)) {
                            if (isset($row['59']) && !empty($row['59'])) {
                                $commercial_mf_unit_raw = trim($row['59']);
                        ...
                        if (isset($row['60']) && !empty($row['60'])) {
                            $commercial_sf = trim($row['60']);
                        }
                        // 61. Singking Fund UOM
                        $commercial_sf_unit = '';
                        if (!empty($commercial_sf)) {
                            if (isset($row['61']) && !empty($row['61'])) {
                                $commercial_sf_unit_raw = trim($row['61']);
                        ...
                        $commercial->parking_lot = $commercial_parking_lot;
                ...
                // 63. Management Developer
                $is_developer = 0;
                if (isset($row['63']) && !empty($row['63'])) {
                    $is_developer_raw = trim($row['63']);
                ...
                if (isset($row['146']) && !empty($row['146'])) {
                    $no_management_raw = trim($row['146']);
                ...
                if (isset($row['149']) && !empty($row['149'])) {
                    $under_10_units_raw = trim($row['149']);

                    if (strtolower($under_10_units_raw) == 'yes') {
                        $under_10_units = true;
                    }
                }

                // 150. Strata under 10 unit remarks
                $under_10_units_remarks = '';
                if (isset($row['150']) && !empty($row['150'])) {
                    $under_10_units_remarks = trim($row['150']);
                }
    
                // 151. Bankruptcy
                $bankruptcy = false;
                if (isset($row['151']) && !empty($row['151'])) {
                    $bankruptcy_raw = trim($row['151']);
                ...
                // 152. Bankruptcy remarks
                $bankruptcy_remarks = '';
                if (isset($row['152']) && !empty($row['152'])) {
                    $bankruptcy_remarks = trim($row['152']);
                }

                // 147. Date Start
                $date_start = '';
                if (isset($row['147']) && !empty($row['147'])) {
                    if(!empty($row['147']['date'])) {
                        $date_start = Carbon::parse($row['147']['date'])->format('Y-m-d');
                    } else {
                        $date_start = Carbon::createFromFormat('d/m/Y', trim($row['147']))->format('Y-m-d');
                    }
                }
                ...
                if (isset($row['148']) && !empty($row['148'])) {
                    if(!empty($row['148']['date'])) {
                        $date_end = Carbon::parse($row['148']['date'])->format('Y-m-d');
                    } else {
                        $date_end = Carbon::createFromFormat('d/m/Y', trim($row['148']['date']))->format('Y-m-d');
                    }
                }
                ...
                if ($create_management) {
                    ...
                    if ($is_developer) {
                        ...
                        if (isset($row['64']) && !empty($row['64'])) {
                            $developer_name = trim($row['64']);
                        }
                        // 65. Address 1
                        $developer_address1 = '';
                        if (isset($row['65']) && !empty($row['65'])) {
                            $developer_address1 = trim($row['65']);
                        }
                        // 66. Address 2
                        $developer_address2 = '';
                        if (isset($row['66']) && !empty($row['66'])) {
                            $developer_address2 = trim($row['66']);
                        }
                        // 67. Address 3
                        $developer_address3 = '';
                        if (isset($row['67']) && !empty($row['67'])) {
                            $developer_address3 = trim($row['67']);
                        }
                        // 68. Address 4
                        $developer_address4 = '';
                        if (isset($row['68']) && !empty($row['68'])) {
                            $developer_address4 = trim($row['68']);
                        }
                        // 69. Postcode
                        $developer_postcode = '';
                        if (isset($row['69']) && !empty($row['69'])) {
                            $developer_postcode = trim($row['69']);
                        }
                        // 70. City
                        $developer_city = '';
                        if (isset($row['70']) && !empty($row['70'])) {
                            $developer_city_raw = trim($row['70']);
                        ...
                        if (isset($row['71']) && !empty($row['71'])) {
                            $developer_state_raw = trim($row['71']);
                        ...
                        if (isset($row['72']) && !empty($row['72'])) {
                            $developer_country_raw = trim($row['72']);
                        ...
                        if (isset($row['73']) && !empty($row['73'])) {
                            $developer_phone_no = trim($row['73']);
                        }
                        // 74. Fax No.
                        $developer_fax_no = '';
                        if (isset($row['74']) && !empty($row['74'])) {
                            $developer_fax_no = trim($row['74']);
                        }
                        // 75. Remarks
                        $developer_remarks = '';
                        if (isset($row['75']) && !empty($row['75'])) {
                            $developer_remarks = trim($row['75']);
                        }
                    ...
                    if ($liquidator) {
                        ...
                        if (isset($row['77']) && !empty($row['77'])) {
                            $liquidator_name = trim($row['77']);
                        }
                        // 78. Address 1
                        $liquidator_address1 = '';
                        if (isset($row['78']) && !empty($row['78'])) {
                            $liquidator_address1 = trim($row['78']);
                        }
                        // 79. Address 2
                        $liquidator_address2 = '';
                        if (isset($row['79']) && !empty($row['79'])) {
                            $liquidator_address2 = trim($row['79']);
                        }
                        // 80. Address 3
                        $liquidator_address3 = '';
                        if (isset($row['80']) && !empty($row['80'])) {
                            $liquidator_address3 = trim($row['80']);
                        }
                        // 81. Address 4
                        $liquidator_address4 = '';
                        if (isset($row['81']) && !empty($row['81'])) {
                            $liquidator_address4 = trim($row['81']);
                        }
                        // 82. Postcode
                        $liquidator_postcode = '';
                        if (isset($row['82']) && !empty($row['82'])) {
                            $liquidator_postcode = trim($row['82']);
                        }
                        // 83. City
                        $liquidator_city = '';
                        if (isset($row['83']) && !empty($row['83'])) {
                            $liquidator_city_raw = trim($row['83']);
                        ...
                        if (isset($row['84']) && !empty($row['84'])) {
                            $liquidator_state_raw = trim($row['84']);
                        ...
                        if (isset($row['85']) && !empty($row['85'])) {
                            $liquidator_country_raw = trim($row['85']);
                        ...
                        if (isset($row['86']) && !empty($row['86'])) {
                            $liquidator_phone_no = trim($row['86']);
                        }
                        // 87. Fax No.
                        $liquidator_fax_no = '';
                        if (isset($row['87']) && !empty($row['87'])) {
                            $liquidator_fax_no = trim($row['87']);
                        }
                        // 88. Remarks
                        $liquidator_remarks = '';
                        if (isset($row['88']) && !empty($row['88'])) {
                            $liquidator_remarks = trim($row['88']);
                        }
                    ...
                    if ($is_jmb) {
                        ...
                        if (isset($row['90']) && !empty($row['90'])) {
                            if(!empty($row['90']['date'])) {
                                $jmb_date_formed = Carbon::parse($row['90']['date'])->format('Y-m-d');
                            } else {
                                $jmb_date_formed = Carbon::createFromFormat('d/m/Y', trim($row['90']))->format('Y-m-d');
                            }
                        }
                        // 91. Certificate Series No
                        $jmb_certificate_no = '';
                        if (isset($row['91']) && !empty($row['91'])) {
                            $jmb_certificate_no = trim($row['91']);
                        }
                        // 92. Name
                        $jmb_name = '';
                        if (isset($row['92']) && !empty($row['92'])) {
                            $jmb_name = trim($row['92']);
                        }
                        // 93. Address 1
                        $jmb_address1 = '';
                        if (isset($row['93']) && !empty($row['93'])) {
                            $jmb_address1 = trim($row['93']);
                        }
                        // 94. Address 2
                        $jmb_address2 = '';
                        if (isset($row['94']) && !empty($row['94'])) {
                            $jmb_address2 = trim($row['94']);
                        }
                        // 95. Address 3
                        $jmb_address3 = '';
                        if (isset($row['95']) && !empty($row['95'])) {
                            $jmb_address3 = trim($row['95']);
                        }
                        // 96. Address 4
                        $jmb_address4 = '';
                        if (isset($row['96']) && !empty($row['96'])) {
                            $jmb_address4 = trim($row['96']);
                        }
                        // 97. Postcode
                        $jmb_postcode = '';
                        if (isset($row['97']) && !empty($row['97'])) {
                            $jmb_postcode = trim($row['97']);
                        }
                        // 98. City
                        $jmb_city = '';
                        if (isset($row['98']) && !empty($row['98'])) {
                            $jmb_city_raw = trim($row['98']);
                        ...
                        if (isset($row['101']) && !empty($row['101'])) {
                            $jmb_office_no = trim($row['101']);
                        }
                        // 102. Fax No.
                        $jmb_fax_no = '';
                        if (isset($row['102']) && !empty($row['102'])) {
                            $jmb_fax_no = trim($row['102']);
                        }
                        // 103. Email
                        $jmb_email = '';
                        if (isset($row['103']) && !empty($row['103'])) {
                            $jmb_email = trim($row['103']);
                        }
                    ...
                    if ($is_mc) {
                        ...
                        if (isset($row['105']) && !empty($row['105'])) {
                            if(!empty($row['105']['date'])) {
                                $mc_date_formed = Carbon::parse($row['105']['date'])->format('Y-m-d');
                            } else {
                                $mc_date_formed = Carbon::createFromFormat('d/m/Y', trim($row['105']))->format('Y-m-d');
                            }
                        }
                        // 106. First AGM Date
                        $mc_first_agm = '';
                        if (isset($row['106']) && !empty($row['106'])) {
                            if(!empty($row['106']['date'])) {
                                $mc_first_agm = Carbon::parse($row['106']['date'])->format('Y-m-d');
                            } else {
                                $mc_first_agm = Carbon::createFromFormat('d/m/Y', trim($row['106']))->format('Y-m-d');
                            }
                        }
                        // 107. Name
                        $mc_name = '';
                        if (isset($row['107']) && !empty($row['107'])) {
                            $mc_name = trim($row['107']);
                        }
                        // 108. Address 1
                        $mc_address1 = '';
                        if (isset($row['108']) && !empty($row['108'])) {
                            $mc_address1 = trim($row['108']);
                        }
                        // 109. Address 2
                        $mc_address2 = '';
                        if (isset($row['109']) && !empty($row['109'])) {
                            $mc_address2 = trim($row['109']);
                        }
                        // 110. Address 3
                        $mc_address3 = '';
                        if (isset($row['110']) && !empty($row['110'])) {
                            $mc_address3 = trim($row['110']);
                        }
                        // 111. Address 4
                        $mc_address4 = '';
                        if (isset($row['111']) && !empty($row['111'])) {
                            $mc_address4 = trim($row['111']);
                        }
                        // 112. Postcode
                        $mc_postcode = '';
                        if (isset($row['112']) && !empty($row['112'])) {
                            $mc_postcode = trim($row['112']);
                        }
                        // 113. City
                        $mc_city = '';
                        if (isset($row['113']) && !empty($row['113'])) {
                            $mc_city_raw = trim($row['113']);
                        ...
                        if (isset($row['116']) && !empty($row['116'])) {
                            $mc_office_no = trim($row['116']);
                        }
                        // 117. Fax No.
                        $mc_fax_no = '';
                        if (isset($row['117']) && !empty($row['117'])) {
                            $mc_fax_no = trim($row['117']);
                        }
                        // 118. Email
                        $mc_email = '';
                        if (isset($row['118']) && !empty($row['118'])) {
                            $mc_email = trim($row['118']);
                        }

                        // 155. Certificate No
                        $certificate_no = 0;
                        if (isset($row['155']) && !empty($row['155'])) {
                            $certificate_no = trim($row['155']);
                        }
                    ...
                    if ($is_agent) {
                        ...
                        if (isset($row['120']) && !empty($row['120'])) {
                            $agent_selected_by = trim($row['120']);
                        }
                        // 121. Agent Name
                        $agent_name = '';
                        if (isset($row['121']) && !empty($row['121'])) {
                            $agent_name = trim($row['121']);
                        }
                        // 122. Address 1
                        $agent_address1 = '';
                        if (isset($row['122']) && !empty($row['122'])) {
                            $agent_address1 = trim($row['122']);
                        }
                        // 123. Address 2
                        $agent_address2 = '';
                        if (isset($row['123']) && !empty($row['123'])) {
                            $agent_address2 = trim($row['123']);
                        }
                        // 124. Address 3
                        $agent_address3 = '';
                        if (isset($row['124']) && !empty($row['124'])) {
                            $agent_address3 = trim($row['124']);
                        }
                        // 125. Address 4
                        $agent_address4 = '';
                        if (isset($row['125']) && !empty($row['125'])) {
                            $agent_address4 = trim($row['125']);
                        }
                        // 126. Postcode
                        $agent_postcode = '';
                        if (isset($row['126']) && !empty($row['126'])) {
                            $agent_postcode = trim($row['126']);
                        }
                        // 127. City
                        $agent_city = '';
                        if (isset($row['127']) && !empty($row['127'])) {
                            $agent_city_raw = trim($row['127']);
                        ...
                        if (isset($row['129']) && !empty($row['129'])) {
                            $agent_country_raw = trim($row['129']);
                        ...
                        if (isset($row['130']) && !empty($row['130'])) {
                            $agent_office_no = trim($row['130']);
                        }
                        // 131. Fax No.
                        $agent_fax_no = '';
                        if (isset($row['131']) && !empty($row['131'])) {
                            $agent_fax_no = trim($row['131']);
                        }
                        // 132. Email
                        $agent_email = '';
                        if (isset($row['132']) && !empty($row['132'])) {
                            $agent_email = trim($row['132']);
                        }
                    ...
                    if ($is_others) {
                        ...
                        if (isset($row['134']) && !empty($row['134'])) {
                            $others_name = trim($row['134']);
                        }
                        // 135. Address 1
                        $others_address1 = '';
                        if (isset($row['135']) && !empty($row['135'])) {
                            $others_address1 = trim($row['135']);
                        }
                        // 136. Address 2
                        $others_address2 = '';
                        if (isset($row['136']) && !empty($row['136'])) {
                            $others_address2 = trim($row['136']);
                        }
                        // 137. Address 3
                        $others_address3 = '';
                        if (isset($row['137']) && !empty($row['137'])) {
                            $others_address3 = trim($row['137']);
                        }
                        // 138. Address 4
                        $others_address4 = '';
                        if (isset($row['138']) && !empty($row['138'])) {
                            $others_address4 = trim($row['138']);
                        }
                        // 139. Postcode
                        $others_postcode = '';
                        if (isset($row['139']) && !empty($row['139'])) {
                            $others_postcode = trim($row['139']);
                        }
                        // 140. City
                        $others_city = '';
                        if (isset($row['140']) && !empty($row['140'])) {
                            $others_city_raw = trim($row['140']);
                        ...
                        if (isset($row['142']) && !empty($row['142'])) {
                            $others_country_raw = trim($row['142']);
                        ...
                        if (isset($row['143']) && !empty($row['143'])) {
                            $others_office_no = trim($row['143']);
                        }
                        // 144. Fax No.
                        $others_fax_no = '';
                        if (isset($row['144']) && !empty($row['144'])) {
                            $others_fax_no = trim($row['144']);
                        }
                        // 145. Email
                        $others_email = '';
                        if (isset($row['145']) && !empty($row['145'])) {
                            $others_email = trim($row['145']);
                        }
                ...
                // 153. Precalculate Plan
                $precalculate_plan = '';
                if (isset($row['153']) && !empty($row['153'])) {
                    $precalculate_plan = trim($row['153']);
                }
                // 154. Buyer Registration
                $buyer_registration = '';
                if (isset($row['154']) && !empty($row['154'])) {
                    $buyer_registration = trim($row['154']);
                }
                // 161. Certificate No
                $certificate_no = '';
                if (isset($row['161']) && !empty($row['161'])) {
                    $certificate_no = trim($row['161']);
                }
                ...
                if (isset($row['157']) && !empty($row['157'])) {
                    $other_details_name = trim($row['157']);
                }
                // 158. Latitude
                $latitude = '';
                if (isset($row['158']) && !empty($row['158'])) {
                    $latitude = trim($row['158']);
                }
                // 159. Longitude
                $longitude = '';
                if (isset($row['159']) && !empty($row['159'])) {
                    $longitude = trim($row['159']);
                }
