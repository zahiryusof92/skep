...
class FinanceAPIController extends BaseController {
    ...
    public function addNewFinance() {
        ...
        try {
            ...
            DB::transaction(function() use($request_params, &$response, $client) {
                /*
                ...
                */
                 ...
                 * Store Building Log
                 */
                $finance = Finance::with(['file.strata'])->find($create_file['data']['id']);
                $building = APIBuilding::firstOrCreate([
                    'client_id' => $client['client']->id,
                    'file_id' => $finance->file->id,
                    'strata_id' => $finance->file->strata->id
                ]);

                $building = APIBuilding::where('file_id', $finance->file->id)->first();
                if(!$building->status) {
                    $response = [
                        'message' => 'This building is not in service!',
                        'error' => true
                    ];
                    return $response;
                }
                $building_log = APIBuildingLog::create([
                    'api_building_id' => $building->id,
                    'finance_file_id' => $finance->id,
                    'remarks' => 'add new finance file'
                ]);
                /**
                 * End Store Building Log
                 */
        ...
        try {
            ...
            DB::transaction(function() use($request_params, &$response, $client) {
                ...
                */
                 ...
                 * Store Building Log
                 */
                $finance = Finance::with(['file.strata'])->find($check_finance['data']['id']);
                $building = APIBuilding::firstOrCreate([
                    'client_id' => $client['client']->id,
                    'file_id' => $finance->file->id,
                    'strata_id' => $finance->file->strata->id
    ...
    public function import() {
         ...
         * Store Building Log
         */
        $finance = Finance::with(['file.strata'])
                        ->where('file_id', $file_no->id)
                        ->where('month', Input::get('import_month'))
                        ->where('year', Input::get('import_year'))
                        ->where('is_deleted', false)
                        ->first();

        if(empty($finance)) {
            $response = [
                'message' => 'Empty Finance File!',
                'error' => true
            ];
            return Response::json(compact('response'), 404);
        }
        ...
        $response =  (new ImportController())->importFinanceFile(true);

        if(in_array($response, ['empty_data', 'empty_file'])) {
            $response = [
                'message' => 'Data empty or finance file not found!',
                'error' => true
            ];
            return Response::json(compact('response'), 404);
        } 
